...
import { getDailyBriefDates, getDailyBriefPreview } from '@/lib/api';
import {
  FileText,
  RefreshCw,
  Loader,
...
export default function DailyBriefPreview() {
  ...
  const fetchDates = useCallback(async () => {
    try {
      const data = await getDailyBriefDates();
  ...
  const fetchBrief = useCallback(async (date: string, p: PersonaId = persona) => {
    ...
    setLoading(true);
    try {
      const data = await getDailyBriefPreview(date, p);
